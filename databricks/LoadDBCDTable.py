...
groupName = spark.sql("""SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(Name, ' ', ''),'(',''),')',''),'-',''),'/',''),'''',''),'Group','')
...
# custColumnListStr = 'FileName,PolID,'+columnListStr[5:]
custColumnListStr = 'ADBID,FileName,'+columnListStr[5:]
...
for f in fileList : #for all the files
    ...
    try :
        ...
        polInfoIndex_obj1 = re.finditer(TargetHeaderPattern1,fileText)
        polInfoIndex1 = [i.start() for i in polInfoIndex_obj1]

        polInfoIndex_obj2 = re.finditer(TargetHeaderPattern2,fileText)
        ...
        if len(InfoIndex) > 0 :
            for i in InfoIndex: #for every occurance of pattern
                ...
                # CustextractionConfigStringN ="'"+f.name+"',"+extractionConfigStringN
                CustextractionConfigStringN ="'"+f.name[:f.name.find('_')]+"',"+"'"+f.name[f.name.find('_')+1:]+"',"+extractionConfigStringN
